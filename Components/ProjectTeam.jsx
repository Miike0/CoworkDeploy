    ...
    return (
        <div className='teamContainer'>
            <div className='d-flex justify-content-between'>
                ...
                <Button className="secondaryButton">Contactar al lider</Button>
            </div>
            <div className='teamWrapper'>
                <h4 className='title mb-4'>{'Integrantes'}</h4>
                {members.length > 0 && members.map((element) => <UserAvatar key={element} image={'none'} name={element} />)}
            </div>

            <div className='teamWrapper'>
                <h4 className='title mb-2 mt-4'>{'Tags'}</h4>
                <Stack direction="row" spacing={1} className="chips-container">
                    {tags.length > 0 && tags.map((tag) => <Chip size="large" key={tag} label={tag} variant="outlined" className='chip' style={{ color: 'white' }} />)}
                </Stack>
            </div>
        </div>
    )
